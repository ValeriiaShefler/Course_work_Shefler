...
from models import TaskCreate, TaskUpdate, TaskResponse, VerifyResponse
from tasks import (
...
async def create_new_task(
    ...
    new_task = await create_task(user["user_id"], task.title, task.description)
    return new_task
...
async def get_task(task_id: int, user: dict = Depends(get_current_user)):
    ...
    task = await get_task_by_id(task_id, user["user_id"])
    if not task:
...
async def delete_existing_task(task_id: int, user: dict = Depends(get_current_user)):
    if not user.get("valid"):
        raise HTTPException(status_code=401, detail=config.ERROR_MESSAGES["invalid_token"])
    
    success = await delete_task(task_id, user["user_id"])
    if not success:
        raise HTTPException(status_code=404, detail=config.ERROR_MESSAGES["task_not_found"])
...
async def delete_all_tasks(user: dict = Depends(get_current_user)):
    if not user.get("valid"):
        raise HTTPException(status_code=401, detail=config.ERROR_MESSAGES["invalid_token"])
    
    await delete_all_user_tasks(user["user_id"])
